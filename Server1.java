...
public class Server1 {
	...
	public static void main(String[] args) throws IOException {
		ipBalancer = args[0];
		String s;
		FileWriter arq = new FileWriter("SD1.txt",false);
		
		

		new Thread() { // listening for a consistency signal from balancer
			
			public void run() {
				try{
					
					ServerSocket serverConsistency = new ServerSocket(33331);
	...
	public static void readFile() {
		...
		try {
			BufferedReader buffRead = new BufferedReader(new FileReader("SD1.txt"));
	        String linha = "";
	        while (true) {
	            if (linha != null) {
	...
	public static void writeFile(String s) {
		...
		try {
			FileWriter arq = new FileWriter("SD1.txt",true);
